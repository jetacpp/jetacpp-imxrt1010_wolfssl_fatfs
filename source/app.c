...
volatile user_action_t g_user_action;
volatile uint32_t g_user_action_time;
...
{
	uint32_t pin = GPIO_GetPinsInterruptFlags(BOARD_USER_BUTTON_GPIO);

    if (g_ticks > g_user_action_time + 3) {
    	if ( ((pin >> 2) & 0x01) == 0 ) {
    		g_user_action = ACTION_ENCRYPT;
    	}
    	else if (((pin >> 5) & 0x01) == 0) {
    		g_user_action = ACTION_DECRYPT;
    	}

    	g_user_action_time = g_ticks;
    }

    GPIO_PortClearInterruptFlags(BOARD_USER_BUTTON_GPIO, 0xffffffff);
...
int main(void)
{
    ...
    uint32_t core = CLOCK_GetCoreFreq();

    /* Configure user button SW4 */
    gpio_pin_config_t sw_config;
    sw_config.direction = kGPIO_DigitalInput;
    sw_config.interruptMode = kGPIO_IntRisingEdge;
    sw_config.outputLogic = 0;

    GPIO_PinInit(BOARD_USER_BUTTON_GPIO, 5, &sw_config);
    GPIO_PortEnableInterrupts(BOARD_USER_BUTTON_GPIO, 1U << 5);

    /* configure button J57-2 */
    GPIO_PinInit(BOARD_USER_BUTTON_GPIO, 2, &sw_config);
    GPIO_PortEnableInterrupts(BOARD_USER_BUTTON_GPIO, 1U << 2);

    EnableIRQ(BOARD_USER_BUTTON_IRQ);

    /* Configure Led status D25 */
    USER_LED_INIT(0);
