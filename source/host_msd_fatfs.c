...
void USB_HostMsdControlCallback(void *param, uint8_t *data, uint32_t dataLength, usb_status_t status)
{
    ...
    if (msdFatfsInstance->runWaitState == kUSB_HostMsdRunWaitSetInterface) /* set interface finish */
    {
        ...
        msdFatfsInstance->runState = kUSB_HOSTMsdRunWaitEncDec;
    }
    controlIng = 0;
...
static void USB_Encrypt_Files(usb_host_msd_fatfs_instance_t *msdFatfsInstance)
{
	FRESULT fatfsCode;
	uint8_t drive[3];
	uint8_t path_in[32], path_out[32];
    DIR dir;

	usb_echo("fatfs mount as logical driver %d......", USBDISK);
	sprintf((char *)&drive[0], "%c:", USBDISK + '0');
	fatfsCode = f_mount(&fatfs, (char const *)&drive[0], 0);
	if (fatfsCode)
	{
		usb_echo("error\r\n");
		return;
    ...
    if ((fatfsCode = f_opendir(&dir, drive)) == 0)
    {
    	FILINFO fileInfo;
    	while (1)
		{
			fatfsCode = f_readdir(&dir, &fileInfo);
    		if (fatfsCode || !strlen(fileInfo.fname))
				break;

    		if (!strstr(fileInfo.fname, ".ENC")) {
    			usb_echo( "Encrypting %s \r\n", fileInfo.fname);
    			sprintf(path_in, "%s/%s", drive, fileInfo.fname);
    			sprintf(path_out, "%s/%s.ENC", drive, fileInfo.fname);
    			aes_encrypt_file(path_in, path_out);
    			processed += fileInfo.fsize;
    			// delete original file
    			f_unlink(path_in);
    		}
		}

    	usb_echo("Encrypting speed: %d bytes/sec \r\n", (int)(processed/(g_ticks-start)));
    }

}

static void USB_Decrypt_Files(usb_host_msd_fatfs_instance_t *msdFatfsInstance)
{
	FRESULT fatfsCode;
	uint8_t drive[3];
	uint8_t path_in[32], path_out[32], name[32];
	DIR dir;
	...
	if ((fatfsCode = f_opendir(&dir, drive)) == 0)
	{
		...
		while (1)
		{
			...
			if ((ch = strstr(name, ".ENC"))) {
				usb_echo( "Decrypting %s \r\n", name);
				sprintf(path_in, "%s/%s", drive, name);
				*ch = 0;
				sprintf(path_out, "%s/%s", drive, name);
				aes_decrypt_file(path_in, path_out);
	  			processed += fileInfo.fsize;
	  			// delete encrypted file
	  			f_unlink(path_in);
			}
		}
    	usb_echo("Decrypting speed: %d bytes/sec \r\n", (int)(processed/(g_ticks-start)));
	}
}
...
void USB_HostMsdTask(void *arg)
{
    ...
    switch (msdFatfsInstance->runState)
    {
        ...
        case kUSB_HOSTMsdRunWaitEncDec:
        	USER_LED_OFF();
        	if (!msdFatfsInstance->aes_init) {
				aes_init();
				msdFatfsInstance->aes_init = 1;
        	}
        	if (g_user_action == ACTION_ENCRYPT){
                msdFatfsInstance->runState = kUSB_HostMsdRunEnc;
                msdFatfsInstance->runWaitState = kUSB_HOSTMsdRunWaitEncDec;
        	}
        	else if(g_user_action == ACTION_DECRYPT) {
                msdFatfsInstance->runState = kUSB_HostMsdRunDec;
                msdFatfsInstance->runWaitState = kUSB_HOSTMsdRunWaitEncDec;
        	}
        	break;

        case kUSB_HostMsdRunEnc:
        	USER_LED_ON();
        	USB_Encrypt_Files(msdFatfsInstance);
        	msdFatfsInstance->runState = kUSB_HOSTMsdRunWaitEncDec;
        	g_user_action = ACTION_NONE;
        	break;

        case kUSB_HostMsdRunDec:
        	USER_LED_ON();
        	USB_Decrypt_Files(msdFatfsInstance);
        	msdFatfsInstance->runState = kUSB_HOSTMsdRunWaitEncDec;
        	g_user_action = ACTION_NONE;
